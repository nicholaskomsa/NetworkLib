...
struct Tf_decoder {
    ...
    char raw[tf_enc_file_size - tf_d_vocab * sizeof(Tf_decoder_item)];
};
...
static void tf_offset_size(const char* json_raw, const char* str, size_t* out_offset, size_t* out_size) {
    char temp[32];
    const char* start = strstr(json_raw, str);
...
static void tf_add(const Tf_add* add) {
...
static void tf_fc(const Tf_fc& fc) {
...
static void tf_ln(const Tf_ln& ln) {
...
static void tf_ln_back(const Tf_ln_back* ln) {
...
static void tf_process(Tf_parameters* parameters, Tf_activations* activations, uint16_t* input, size_t input_size, uint16_t* out_token, bool is_training, Tf_gradients* gradients, Tf_activations_back* activations_back, uint16_t* expected) {
    ...
    for (int layer_i = 0; layer_i < tf_n_layers; layer_i++) {
        tf_ln({
            parameters->h[layer_i].ln_1.weight,
            parameters->h[layer_i].ln_1.bias,
            layer_i == 0 ? (float*)activations->embedding.out : (float*)activations->h[layer_i - 1].res_2.out,
            (float*)activations->h[layer_i].ln_1.r_std,
            (float*)activations->h[layer_i].ln_1.mean,
            (float*)activations->h[layer_i].ln_1.out,
            tf_d_model,
            input_size
            });
        ...
        tf_fc({
            parameters->h[layer_i].attn.c_attn.weight,
            parameters->h[layer_i].attn.c_attn.bias,
            (float*)activations->h[layer_i].ln_1.out,
            (float*)activations->h[layer_i].attn.c_attn.out,
            tf_d_model,
            3 * tf_d_model,
            input_size,
            });
        ...
        tf_fc({
            parameters->h[layer_i].attn.c_proj.weight,
            parameters->h[layer_i].attn.c_proj.bias,
            (float*)activations->h[layer_i].attn.z.out,
            (float*)activations->h[layer_i].attn.c_proj.out,
            tf_d_model,
            tf_d_model,
            input_size,
            });
        ...
        tf_add({
            layer_i == 0 ? (float*)activations->embedding.out : (float*)activations->h[layer_i - 1].res_2.out,
            (float*)activations->h[layer_i].attn.c_proj.out,
            (float*)activations->h[layer_i].res_1.out,
             input_size * tf_d_model,
            });


        tf_ln({
            parameters->h[layer_i].ln_2.weight,
            parameters->h[layer_i].ln_2.bias,
            (float*)activations->h[layer_i].res_1.out,
            (float*)activations->h[layer_i].ln_2.r_std,
            (float*)activations->h[layer_i].ln_2.mean,
            (float*)activations->h[layer_i].ln_2.out,
            tf_d_model,
            input_size,
            });
        ...
        tf_fc({
            parameters->h[layer_i].mlp.c_fc.weight,
            parameters->h[layer_i].mlp.c_fc.bias,
            (float*)activations->h[layer_i].ln_2.out,
            (float*)activations->h[layer_i].mlp.c_fc.out,
            tf_d_model,
            4 * tf_d_model,
            input_size,
            });

        constexpr auto M_SQRT1_2 = 1.0f / std::numbers::sqrt2;
        {
        ...
        tf_fc({
            parameters->h[layer_i].mlp.c_proj.weight,
            parameters->h[layer_i].mlp.c_proj.bias,
            (float*)activations->h[layer_i].mlp.gelu.out,
            (float*)activations->h[layer_i].mlp.c_proj.out,
            4 * tf_d_model,
            tf_d_model,
            input_size,
            });
        ...
        tf_add({
            (float*)activations->h[layer_i].res_1.out,
            (float*)activations->h[layer_i].mlp.c_proj.out,
            (float*)activations->h[layer_i].res_2.out,
            input_size * tf_d_model,
            });
    ...
    tf_ln({
        ...
        (float*)activations->h[11].res_2.out,
        (float*)activations->ln_f.r_std,
        (float*)activations->ln_f.mean,
        (float*)activations->ln_f.out,
        tf_d_model,
        input_size,
        });
...
#define tf_align(x) (((x) + 255) & ~(size_t)0xff)
    ...
    total_offset += tf_align(sizeof(Tf_decoder));
    ...
    total_offset += tf_align(sizeof(Tf_parameters));
    size_t activations_offset = total_offset;
    total_offset += tf_align(sizeof(Tf_activations));
    size_t gradients_offset = total_offset;
    total_offset += tf_align(sizeof(Tf_gradients));
    size_t activations_back_offset = total_offset;
    total_offset += tf_align(sizeof(Tf_activations_back));
#undef tf_align

    fprintf(stderr, "requires %lu mib\n", total_offset >> 20);

    char* tf_raw = (char*)_aligned_malloc(256, total_offset);

    assert(tf_raw && "allocation failed");

    Tf_decoder* decoder = (Tf_decoder*)(tf_raw + decoder_offset);
    uint16_t* data = (uint16_t*)(tf_raw + data_offset);
    char* json_raw = (char*)(tf_raw + json_raw_offset);
    char* parameters_raw = (char*)(tf_raw + parameters_raw_offset);
    Tf_parameters* parameters = (Tf_parameters*)(tf_raw + parameters_offset);
    Tf_activations* activations = (Tf_activations*)(tf_raw + activations_offset);
    Tf_gradients* gradients = (Tf_gradients*)(tf_raw + gradients_offset);
    Tf_activations_back* activations_back = (Tf_activations_back*)(tf_raw + activations_back_offset);
    ...
    {
        std::string enc_path = std::format("{}/assets/enc", base_path);

        FILE* f = fopen(enc_path.c_str(), "r");
        assert(f);
        ...
        size_t read_size = tf_d_vocab * sizeof(Tf_decoder_item);
    ...
    {
        std::string data_path = std::format("{}/assets/data", base_path);
    ...
    {
        std::string parameters_path = std::format("{}/assets/model.safetensors", base_path);
        // https://huggingface.co/openai-community/gpt2?show_file_info=model.safetensors

        FILE* f = fopen(parameters_path.c_str(), "r");
        assert(f);
